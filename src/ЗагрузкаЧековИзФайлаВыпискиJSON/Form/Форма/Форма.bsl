...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.СохранитьВыпискуJSONВФайл.Видимость = Ложь;
...
Процедура ПриОткрытии(Отказ)
	...
	ПримененныеНастройки = Новый Структура("Логин, Пароль");
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "Записана операция" И Параметр.ВидДокумента = "Расход" Тогда
		...
		лНайденыеСтроки = Объект.СписокЧеков.НайтиСтроки(лОтбор);
		Для Каждого стр Из лНайденыеСтроки Цикл
...
Процедура ПериодОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если Период.Вариант = ВариантСтандартногоПериода.ПроизвольныйПериод Тогда
		...
		Период.ДатаНачала    = ?(ЗначениеЗаполнено(Период.ДатаНачала),
		...
		Период.ДатаОкончания = ?(ЗначениеЗаполнено(Период.ДатаОкончания),
			...
			КонецГода(ОбщегоНазначенияКлиент.ДатаСеанса())
		);
...
Процедура ПрименитьНастройки(Команда)
	...
	лОтказ = Ложь;
	//ПроверитьОбязательныеНастройки(лОтказ);
...
&НаКлиенте
Процедура СохранитьВыпискуJSONВФайл(Команда)
	
	лИмяФайла = СтрШаблон(
		НСтр("ru = '%1_%2'"),
		Формат(Период.ДатаНачала, "ДФ=yyyy-MM-dd"),
		Формат(Период.ДатаОкончания, "ДФ=yyyy-MM-dd")
	);
	
	СохранитьJSONВФайл(ВыпискаJSON, лИмяФайла);
...
Процедура ЗагрузитьСписокЧеков(Команда)
	...
	Если НЕ ЗначениеЗаполнено(Период) Тогда
		ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(
			лОшибки,
			"Период",
			ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(,, "Период"),
			Неопределено
		);
	КонецЕсли;
	
	ОбщегоНазначенияКлиентСервер.СообщитьОшибкиПользователю(лОшибки, лОтказ);
...
&НаСервере
Процедура ЗагрузитьСписокЧековНаСервере(Знач пСодержимоеФайла = Неопределено)
	...
	ИначеЕсли пСодержимоеФайла <> Неопределено Тогда
	...
	Иначе 
		...
		лДатаОкончания = Мин(Период.ДатаОкончания, ТекущаяДатаСеанса());
		
		лФорматнаяСтрока = "ДФ=yyyy-MM-ddTHH:mm:ss";
		
		лАдресРесурса = СтрШаблон("/v1/extract?dateFrom=%1&dateTo=%2&sendToEmail=0&fileType=json",
			Формат(Период.ДатаНачала, лФорматнаяСтрока),
			Формат(лДатаОкончания, лФорматнаяСтрока)
		);
		
		лАдресФайла = ПолучитьОтветОтФНС(лАдресРесурса, Логин, Пароль);
		
		Если лАдресФайла = Неопределено Тогда
			Возврат;
		КонецЕсли;
		
		лСписокЧеков = ПолучитьОтветОтФНС(лАдресФайла.url, Логин, Пароль);
		Если лСписокЧеков = Неопределено Тогда 
			Возврат; 
		КонецЕсли;
	...
	Для Каждого лЧек Из лСписокЧеков Цикл
		...
		лНовыйЧек.Дата 		= лТелоЧека.dateTime;
		...
		лНовыйЧек.ИНН 		= СокрЛП(лТелоЧека.userInn);
...
&НаКлиенте
Процедура ПроверитьОбязательныеНастройки(Отказ)
	...
	Если ПустаяСтрока(Логин) Тогда
		ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(
			лОшибки,
			"Логин",
			ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(,, "Логин"),
			Неопределено
		);
	КонецЕсли;
	...
	Если ПустаяСтрока(Пароль) Тогда
		ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(
			лОшибки,
			"Пароль",
			ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(,, "Пароль"),
			Неопределено
		);
	КонецЕсли;
	
	ОбщегоНазначенияКлиентСервер.СообщитьОшибкиПользователю(лОшибки, Отказ);

КонецПроцедуры

&НаСервереБезКонтекста
Функция ПолучитьОтветОтФНС(АдресРесурса, Логин, Пароль)
	...
	лЗаголовки.Вставить("Authorization", "Basic " + ЗашифроватьBase64(Логин + ":" + Пароль, "US-ASCII"));
	лЗаголовки.Вставить("Device-Id", лСистемнаяИнформация.ИдентификаторКлиента);
	лЗаголовки.Вставить("Device-OS", лСистемнаяИнформация.ВерсияОС);
	
	лСервер = "proverkacheka.nalog.ru";
	лПорт 	= 8888;
	лПредставлениеОшибки = "";
	
	лОтвет = RESTКлиентСервер.ВыполнитьHTTPЗапрос(
		"GET",
		лСервер,
		АдресРесурса,
		Неопределено,,
		...
		Ложь,,
		лПорт
		//,,,, лПредставлениеОшибки// Закомментирвать на версии 2.0.34.8
	);
	...
	лОтветСтрокой = лОтвет.ПолучитьТелоКакСтроку();
	
	лУспешно = 200;
	Если НЕ лОтвет.КодСостояния = лУспешно Тогда
		лПредставлениеОшибки = СтрШаблон(
			НСтр("ru = 'Ошибка (код %1) при выполнении запроса к серверу %2%3:%4%5'"),
			лОтвет.КодСостояния,
			лСервер,
			АдресРесурса,
			Символы.ПС,
	...
	Попытка
		лРезультат = ОбъектИзСтрокиJSON(лОтветСтрокой);
	Исключение
	    лПредставлениеОшибки = СтрШаблон(
			НСтр("ru = 'Не удалось прочитать в структуру JSON из текста ответа сервера:%1%2'"),
			Символы.ПС,
			лОтветСтрокой
		);
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(лПредставлениеОшибки);
...
&НаКлиентеНаСервереБезКонтекста 
Функция ЗашифроватьBase64(Знач Строка, Знач Кодировка)
	
	лИмяВременногоФайла = ПолучитьИмяВременногоФайла();
	
	лЗаписьТекста = Новый ЗаписьТекста(лИмяВременногоФайла, Кодировка);
	лЗаписьТекста.Записать(Строка);
	лЗаписьТекста.Закрыть();
	
	лДвоичные = Новый ДвоичныеДанные(лИмяВременногоФайла);
	лРезультат = Base64Строка(лДвоичные);
	Если Лев(лРезультат, 4) = "77u/" Тогда
		лРезультат = Сред(лРезультат, 5);
	КонецЕсли;
	лРезультат = СтрЗаменить(лРезультат, Символы.ПС, "");
...
Процедура ПодобратьДокументыРасхода()
	...
	Для Каждого стр1 Из лТаблицаДляУточнения Цикл
		...
		лНайденыеСтроки = Объект.СписокЧеков.НайтиСтроки(лОтбор);
		Для Каждого стр2 Из лНайденыеСтроки Цикл
...
Процедура ПодобратьСтатьиРасхода()
	...
	Пока Выборка.Следующий() Цикл
		ЗаполнитьЗначенияСвойств(лОтбор, Выборка);
		лНайденыеСтроки = Объект.СоставЧека.НайтиСтроки(лОтбор);
		Для Каждого стр Из лНайденыеСтроки Цикл
